...
def weekly_commit(user_id: int, start: datetime):
    ...
    query = text(
        "SELECT timestamp, status FROM time_logs WHERE user_id = :user_id and :start < timestamp;"
    )
...
def update_total_time(user_id: int):
        ...
        .filter(TimeLog.user_id == user_id, TimeLog.status == "end")
        .order_by(desc(TimeLog.timestamp))
        .first()
    )
    start = (
        session.query(TimeLog)
        .filter(TimeLog.user_id == user_id, TimeLog.status == "start")
        .order_by(desc(TimeLog.timestamp))
        .first()
    )
